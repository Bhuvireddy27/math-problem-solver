...
# Extracting values safely and converting to proper types
    test_type = data.get('test_type')
    
    # Ensure sample_data is a list of floats
    sample_data = list(map(float, data.get('sample_data', [])))
    population_mean = float(data.get('population_mean', 0))

    # Validate input
    if not sample_data or not isinstance(population_mean, (int, float)):
